 ...
 * in.
 */
export async function pluginBytes(pluginId: string): Promise<Uint8Array> {
  const entry = await entryFor(pluginId);
  return bytesFor(entry.module, pluginId, 'module');
}
...
export async function assetBytes(sha256: string): Promise<Uint8Array> {
  ...
  for (const plugin of manifest.plugins) {
    ...
    if (asset) return bytesFor(asset, plugin.id, 'asset');
  }
...
export async function installPlugin(pluginId: string): Promise<void> {
  ...
  await bytesFor(entry.module, pluginId, 'module');
  for (const asset of entry.assets) await bytesFor(asset, pluginId, 'asset');
}

export async function installedState(pluginId: string): Promise<InstalledState> {
  const entry = await entryFor(pluginId);
  const held = new Map((await blobsForPlugin(pluginId)).map((row) => [row.sha256, row]));
...
export async function removePlugin(pluginId: string): Promise<void> {
  const manifest = await verifiedManifest();
  const mine = await blobsForPlugin(pluginId);

  const shared = new Set<string>();
  for (const plugin of manifest.plugins) {
    if (plugin.id === pluginId) continue;
    if ((await blobsForPlugin(plugin.id)).length === 0) continue;
  ...
  await evictFromCaches(manifest.plugins.filter((plugin) => plugin.id === pluginId));
}

async function entryFor(pluginId: string): Promise<ManifestPlugin> {
  let manifest;
  ...
  const entry = manifest.plugins.find((plugin) => plugin.id === pluginId);
  if (!entry) {
...
async function bytesFor(
  ...
  const work = obtain(file, pluginId, kind).finally(() => pending.delete(file.sha256));
...
async function obtain(
  ...
  await putBlob({ sha256: file.sha256, pluginId, kind, bytes, storedAt: Date.now() });

//! Two clients, one log, one deck — version 2.
//!
...
//! proofs, is `tabla-deal/tests/letras_v2.rs`. The dependency points that way
...
use crate::v2::Plugin;
use crate::v2::game::{Action, Laid, Letras as Game, Move, Private, State, View, config_for};
...
#[test]
fn a_successful_challenge_takes_back_the_play_and_its_refill() {
    ...
    table
        .play(Action::Challenge)
        .expect("the opponent may challenge");

    let after = table.view(mover);
    // Board, score, tiles and the refill all come back.
    ...
    assert_eq!(after.bag, u16::from(TILE_TOTAL) - 14);
}

#[test]
fn a_failed_challenge_costs_the_challenger_the_turn_they_spent_on_it() {
    let word = Table::opening_word(3);
    let mut table = Table::with_words(&[word.as_str()]);
    table.open();
    ...
    let mover = table.to_move();
    table.play_from_rack(3);
    let scored = table.view(mover).scores[mover as usize];

    table
        .play(Action::Challenge)
        .expect("the opponent may challenge");

    // The play stands, and the refill it earned arrived with the challenge —
    // a challenge is an ordinary entry and carries what it owes.
    assert_eq!(table.view(mover).scores[mover as usize], scored);
    assert_eq!(table.view(mover).rack_positions.len(), 7);

    // Nothing further is owed. The turn spent challenging was the price, so
    // play simply carries on with the mover.
    assert_eq!(table.to_move(), mover);
    assert_eq!(table.view(mover).phase, "play");
}

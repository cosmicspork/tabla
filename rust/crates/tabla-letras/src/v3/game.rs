//! Letras with words checked as they are played.
//!
//! Same board, same tiles, same word list, same deck as [`crate::v2`]. What
//! changed is when the word list is consulted.
//!
//! ## Why the challenge went
//!
//! Under the tournament rule a play was legal the moment it was geometrically
//! sound, and whether it was a *word* was a question the opponent had to raise
//! and pay for if they raised it wrongly. That rule earns its keep across a
//! physical board, where the alternative is one player leafing through a
//! dictionary while the other waits.
//!
//! It earns nothing here. Both devices already hold the identical word list —
//! pinned by hash in the invite, checked before the rules ever see it — so
//! "is this a word" is a question both of them can answer instantly and
//! identically. Leaving it unanswered until someone objected meant a game could
//! be won with letters nobody claims are English, by an opponent who was not
//! paying attention. The deal made playing a tile you were not dealt impossible
//! rather than punishable; this does the same for words.
//!
...
impl GamePlugin for Letras {
    ...
    fn validate_move(
        ...
        match (state.expected(), &mv.action) {
            ...
            (Expect::Play, Action::Place { placements }) => {
                ...
                // And every word it makes has to be one. Both devices run this
                // — the mover before sealing the entry, the opponent while
                // replaying it — against the identical list, so they cannot
                // disagree about the answer.
                if let Some(word) = first_unknown_word(&state.board, &laid, assets)? {
                    return Err(PluginError::NotAWord { word });
                }
    ...
    fn apply_move(mut state: State, mv: &Move, _assets: &[u8]) -> Result<State, PluginError> {
        ...
        // action it carries. There is no longer any move that takes a play back
        // off the board, so there is nothing that could cancel a refill.
...
/// The first word a play would make that is not in the list, if any.
///
/// Named rather than merely counted, because "IAEU is not in the word list" is
/// something a player can act on and "that is not a word" is not — the play may
/// have made three words and only one of them is the problem.
///
/// Reading the list here is what makes this checkable by both devices: the
/// bytes were pinned by hash in the invite and verified before the rules saw
/// them, so the mover and the opponent cannot reach different answers.
fn first_unknown_word(
    board: &[Option<Placed>],
    ...
    for word in words_formed(board, placements) {
        ...
        if !dictionary.contains(&text) {
            return Ok(Some(String::from_utf8_lossy(&text).to_uppercase()));
        }
    }
